...
take a considerable bite out of cpu resources on an intermittent basis.

=head3 rDNS via BIND forward only

Set up dnsbls as above with dnsbls running on port 8653 as above. You may
...
the BIND daemon to do zone transfers as in the previous method.

The disadvantage to this method is that no zone file is immediately
available for export. A zone file can be created on demand by issuing a SIG_USR2 
to the dnsbls daemon. Updating the zone file in this manner is best left to
a cron job. 

See example CRON shell script in the ./contrib directory of this
distribution.
